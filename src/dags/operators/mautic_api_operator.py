...
class MauticApiOperator(BaseOperator):
    ...
    def merge_votes_and_contacts(self):
        ...
        print(f"Airflow will request {len(mtc_ids)} contacts on mautic api")
    ...
    def _get_contact(self, mtc_id):
        ...
        headers = {"Authorization": f"Basic cGVuY2lsbGFiczpAUGVuYzFsTGFiNQ=="}
        response = requests.get(url, headers=headers)
        return response.json()["contact"]

    def _get_url(self, mtc_id):
        return (
            f"{self.connection.schema}://{self.connection.host}/api/contacts/{mtc_id}"
        )
    ...
    def _merge(self, df, contact, mtc_id):
        ...
        df.loc[df["email"] == f"{mtc_id}-mautic@mail.com", "mtc_email"] = mautic_email
        df.loc[
            df["email"] == f"{mtc_id}-mautic@mail.com", "mtc_first_name"
        ] = first_name
        df.loc[df["email"] == f"{mtc_id}-mautic@mail.com", "mtc_last_name"] = last_name
        return df
